...
tpopl$value <- cton(tpopl$value)/1000
...
tpop<-read.csv(url, header=TRUE, skip=0, colClasses="character")
...
pop2010 <- tpop %>% select(-`2010decennial1`, -`2010estbase`) %>%
  ...
  mutate(year=as.numeric(gsub("[[:alpha:]]", "", year)),
         esttype="intercensal",
         value=cton(value)/1e3) # put in thousands
ht(pop2010)
count(pop2010, year)
...
qplot(year, log(value), data=filter(popst, stabbr %in% c("CA","FL","IL","NY","TX","VA")), colour=stabbr, geom=c("point","line"))

qplot(year, log(value), data=filter(popall, stabbr=="US" & esttype=="intercensal"), geom=c("point","line"))

qplot(year, log(value), data=filter(popall, stabbr=="US" & grepl("decennial",esttype)), geom=c("point","line"))
